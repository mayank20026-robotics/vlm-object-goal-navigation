...
class YoloSearchCommander(Node):
    def __init__(self, user_query):
        ...
        self.angular_speed = 0.5 # Increased slightly because P-controller will slow it down
    ...
    # --- UPDATED: Closed-loop turning trigger ---
    def start_turn_45(self):
        self.turning = True
        # Calculate target yaw and wrap it safely between -PI and +PI
        ...
        self.get_logger().info(f"↪️ Turning 45 degrees (Turn {self.turn_count}/{self.max_turns_before_move})...")

    def move_to_new_vantage_point(self):
        self.get_logger().info("🚶 Full 360 sweep complete. Moving to a new vantage point...")
        goal = PoseStamped()
    ...
    def control_loop(self):
        ...
        # --- UPDATED: Closed-loop turning logic ---
        if self.turning:
            # Calculate angular error and wrap to [-PI, PI]
            error = self.target_yaw - self.current_yaw
            error = math.atan2(math.sin(error), math.cos(error))

            if abs(error) > 0.05: # Stop if we are within ~2.8 degrees of target
                # Proportional Control: Slows down as error gets smaller
